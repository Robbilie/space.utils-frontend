...
class SearchBar extends React.Component {
  ...
  constructor (props) {
    ...
    if (this.props.search)
      this.search(this.props.search, true);
    else
  ...
  async request_search (search, init) {
    ...
    let { obj: search_data } = await client.apis.Search.get_search({ search, strict: !!init, categories: this.props.categories });
    if ([].concat(...Object.values(search_data)).length == 0)
      return this.setState({
        results: [["No results", []]]
      });
    let { obj } = await client.apis.Universe.post_universe_names({ ids: [].concat(...Object.values(search_data).map(val => val.slice(0, this.props.limit))) });
    const lookup = obj.reduce((p, { id, name }) => !(p[id] = name) || p, {});
    let results = Object.entries(search_data).map(([name, ids]) => ([name, ids.slice(0, this.props.limit).map(id => ({ id, name: lookup[id] }))]));
